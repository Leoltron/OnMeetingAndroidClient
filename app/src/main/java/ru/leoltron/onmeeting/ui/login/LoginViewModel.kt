...
class LoginViewModel(private val onMeetingApi: IOnMeetingApi) : ViewModel() {
        ...
        onMeetingApi.getUser(username).enqueue(object : SimpleCallback<UserModel>() {
            override fun onResponse(call: Call<UserModel>, response: Response<UserModel>) {
                if (response.code() == 404) {
                    onMeetingApi.register(username, password).enqueue(object : SimpleCallback<ResponseBody>() {
    ...
    private fun getResultFromBody(responseBody: ResponseBody?): String {
